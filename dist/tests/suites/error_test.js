...
function suite() {
    ...
    it('should throw EncryptorError when cipher with not valid Key', done => {
    ...
    it('should throw EncryptorError when cipher with not valid algorithm', done => {
        try {
            new Encryptor({
                key,
                key_length: 31
    ...
    it('should throw EncryptorError when decipher not valid Json', done => {
    ...
    it('should throw EncryptorError when decipher invalid MAC signature', done => {
        const encryptor = new Encryptor({ key });
    ...
    it('should throw EncryptorError when decipher with invalid Payload', done => {
        const encryptor = new Encryptor({ key });
        const encrypted = encryptor.encryptSync(one_object);
        let fake = Buffer.from(encrypted, 'base64').toString('utf8');
        fake = JSON.parse(fake);
    ...
    it('should throw EncryptorError when decipher with invalid iv length', done => {
        const encryptor = new Encryptor({ key });
        const encrypted = encryptor.encryptSync(one_object);
        let fake = Buffer.from(encrypted, 'base64').toString('utf8');
        fake = JSON.parse(fake);
